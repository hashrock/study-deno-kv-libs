...
export function createValue(
  partialValue: Partial<$.com.deno.kv.backup.BackupSnapshotRange>,
): $.com.deno.kv.backup.BackupSnapshotRange {
  return {
    ...getDefaultValue(),
    ...partialValue,
  };
...
export function encodeJson(
  value: $.com.deno.kv.backup.BackupSnapshotRange,
): unknown {
  const result: any = {};
...
export function decodeJson(
  ...
  result.dataList = value.dataList?.map((value: any) => decodeJson_1(value)) ??
    [];
  result.metadataList =
    value.metadataList?.map((value: any) => decodeJson_1(value)) ?? [];
  return result;
}
